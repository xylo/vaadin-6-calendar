...
public class Schedule extends AbstractComponent {
	...
	public static class CalendarFormat {
		...
		public static final CalendarFormat FORMAT_12H = new CalendarFormat("FORMAT12");
		public static final CalendarFormat FORMAT_24H = new CalendarFormat("FORMAT24");
	...
	public void paintContent(PaintTarget target) throws PaintException {
		...
		if (durationInDays > 60) {
			throw new PaintException("Daterange is too big (max 60) = " + durationInDays);
		}

		target.addAttribute("format24h", currentFormat == CalendarFormat.FORMAT_24H);
		...
		target.addAttribute("now", df_date.format(now) + " " + df_time.format(now));
		...
		while (currentCalendar.getTime().compareTo(lastDateToShow) < 1) {
			target.startTag("day");
			target.addAttribute("date", df_date.format(currentCalendar.getTime()));
			target.addAttribute("fdate", weeklyCaptionFormat.format(currentCalendar.getTime()));
			target.addAttribute("dow", currentCalendar.get(Calendar.DAY_OF_WEEK));
			target.addAttribute("w", currentCalendar.get(Calendar.WEEK_OF_YEAR));
			target.endTag("day");
	...
	protected void paintEvent(int i, PaintTarget target) throws PaintException {
		...
		target.addAttribute("description", e.getDescription() == null ? "" : e.getDescription());
		target.addAttribute("extracss", e.getStyleName() == null ? "" : e.getStyleName());
	}
			...
			if (message != null && message.length() > 10) {
				...
				if (splitted.length == 2) {
					...
					try {
						...
						if (index >= 0 && index < events.size() && events.get(index) != null) {
