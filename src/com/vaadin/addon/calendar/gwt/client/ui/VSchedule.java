...
public class VSchedule extends Composite implements Paintable {
	...
	public void updateFromUIDL(UIDL uidl, ApplicationConnection client) {
		...
		if (daysCount > 7) {
			if (hideWeekends) {
				nameToolbar.setDayNames(new String[] { dayNames[1], dayNames[2], dayNames[3], dayNames[4], dayNames[5] });
			} else if (firstDayOfWeek == 2) {
				nameToolbar.setDayNames(new String[] { dayNames[1], dayNames[2], dayNames[3], dayNames[4], dayNames[5], dayNames[6], dayNames[0] });
			} else {
				nameToolbar.setDayNames(new String[] { dayNames[0], dayNames[1], dayNames[2], dayNames[3], dayNames[4], dayNames[5], dayNames[6] });
			}
	...
	private boolean isEventInDay(Date eventWhen, Date eventTo, Date gridDate) {
		if (eventWhen.compareTo(gridDate) <= 0 && eventTo.compareTo(gridDate) >= 0) {
			return true;
		}
		return false;
	}
	...
	protected ArrayList<ScheduleEvent> getEvents(UIDL childUIDL) {
		...
		for (int i = 0; i < eventCount; i++) {
			...
			e.setFromDatetime(dateformat_datetime.parse(datefrom + " " + timefrom));
	...
	public void updateWeekGrid(int daysCount, UIDL daysUidl, Date today) {
		...
		for (int i = 0; i < daysCount; i++) {
			...
			dayToolbar.add(dayNames[dayOfWeek - 1], date, localized_date_format, isToday ? "today" : null);
	...
	private void updateMonthGrid(int daysCount, UIDL daysUidl, Date today) {
		...
		for (int i = 0; i < daysCount; i++) {
			...
			cell.setDate(d);
			int dayOfMonth = d.getDate();
			if (dayOfMonth == 1 || (hideWeekends && x == 0 && (dayOfMonth == 2 || dayOfMonth == 3))) {
	...
	public void setHeight(String newHeight) {
		if (!this.height.equals(newHeight)) {
			...
			this.intHeight = Integer.parseInt(newHeight.substring(0, newHeight.length() - 2));
	...
	private void recalculateHeights() {
		if (monthGrid != null) {
			monthGrid.updateCellSizes(intWidth - MONTHLY_WEEKTOOLBARWIDTH, intHeight - MONTHLY_DAYTOOLBARHEIGHT);
	...
	public void setWidth(String newWidth) {
		if (!this.width.equals(newWidth)) {
			this.width = newWidth;
			this.intWidth = Integer.parseInt(newWidth.substring(0, newWidth.length() - 2));
			recalculateWidths();
		}
	}
